...
//		numOfCollisions = 0;
	...
	public int hashFunction( int home , int counter ){
		int x = (home + randomPermutations[counter] ) % currentSize;
		return x;
	}//end method.
	
	private void randomProbing( int size){
		
		randomPermutations = new int[size];
		Random r = new Random();
		randomPermutations[0] = 0;
		boolean[] visited = new boolean[size];
		
		int counterCheck = 1;
		while( counterCheck != size ){
			int num = r.nextInt(size-1) + 1;
			if( !visited[num] ){
				visited[num] = true;
				randomPermutations[counterCheck] = num;
				counterCheck++;
			}
		}//end while.
