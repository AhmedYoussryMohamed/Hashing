...
public class DoubleHashing<K, V> implements HashTable<K, V> {
	...
	public void put(K key, V value) {
		...
		while ( array[nextKey] != null ) {
			...
			counter++;
			nextKey = hashFunction( key.hashCode() ,home ,counter);
		}// end while.
...
//		numOfCollisions = 0;
	...
	public int hashFunction(int key , int home , int counter ){
		int x = key / currentSize;
		int y = currentSize / 2;
		int z = (x % y) * 2;
		int linearProbeConstant = z + 1;
		int m = (counter * linearProbeConstant) % currentSize;
		return m;
	}//end method.
